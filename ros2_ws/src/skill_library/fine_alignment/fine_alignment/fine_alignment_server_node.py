...
class SearchState(Enum):
    ...
    PANEL_TOUCHING: Panel surface touched. Set surface height.
...
class SpiralSearchControllerNode(Node):
    def __init__(self):
        ...
        self._touch_z_history = deque(maxlen=self._steady_state_len)
        ...
        # Service for zeroing the FT sensor
        self.FT_sensor_zeroing_client = self.create_client(Trigger, 'optoforce/zero')
        ...
        # Create client for control the gripper and setPosition
    ...
    def _run_goal(self, goal_handle):
        ...
        while True:
            ...
            if item.result_status == FineAlignment.Result.SUCCESS:
                ...
                # Moving 5cm backward, keep the orientation
                R_B_G = self.T_B_G[:3, :3]
                orient_deg = R.from_matrix(R_B_G).as_euler('xyz', degrees=False)

                new_position = (self.T_B_G @ np.array([0.0, 0.0, -0.05, 1.0]))[:3]
                exit_pose = np.concatenate([new_position, orient_deg])

                self.get_logger().info(f"Target pose: {exit_pose}")

                self._set_position(positions=exit_pose.tolist())
    ...
    def _wait_until_ready(self, goal_handle) -> bool:
        ...
        while not self.FT_sensor_zeroing_client.wait_for_service(timeout_sec=1.0):
            if goal_handle.is_cancel_requested:
                return False
            if time.monotonic() >= deadline:
                self.get_logger().error('Timed out waiting for optoforce/zero service')
                return False
            self.get_logger().info('Service not available, waiting...')

        self.get_logger().info('Calling optoforce/zero...')
        future = self.FT_sensor_zeroing_client.call_async(Trigger.Request())
        ...
        while not future.done():
            if goal_handle.is_cancel_requested:
                ...
                return False
            if time.monotonic() >= deadline:
                self.get_logger().error('Timed out waiting for optoforce/zero response')
                future.cancel()
                return False
            time.sleep(0.05)
        ...
        if response is None:
            self.get_logger().error('Failed to call optoforce/zero service')
            return False
        if not (response.success and response.message.startswith('SENSOR_OK')):
            self.get_logger().error(f"FT sensor zeroing failed: {response.message}")
            return False

        self.get_logger().info('FT sensor zeroed')
    ...
    def _make_result(self, status: int, message: str):
        result = FineAlignment.Result()
        result.status = status
        result.message = message

        result.pose_valid = bool(self.feedback_is_avaliable)
        if result.pose_valid:
            final_pos_base = self.T_B_G[:3, 3]
            final_orient_deg = R.from_matrix(self.T_B_G[:3, :3]).as_euler('xyz', degrees=False)
            result.x_m, result.y_m, result.z_m = final_pos_base.tolist()
            result.rx_deg, result.ry_deg, result.rz_deg = final_orient_deg.tolist()
        else:
            result.x_m = result.y_m = result.z_m = 0.0
            result.rx_deg = result.ry_deg = result.rz_deg = 0.0
        return result
    ...
    def _set_position(self, positions=[0.2, -0.4, 0.35, 1.57, 0.0, 1.57], velocity=0.1, acc_time=0.5, blend_percentage=100, fine_goal=False):   
    ...
    def _ctrl_tick(self):
        ...
        fx = self.force_observe.x
        fy = self.force_observe.y
        ...
        if self.state == SearchState.WAITING_TOUCH:
            if abs(self.touch_force - fz) <= self.touch_force_tol_n:
                self.state = SearchState.PANEL_TOUCHING
                self.get_logger().info(
                    f"[TOUCH] contact detected (Fz={fz:.3f}N) -- starting spiral search"
                )
                self._feedback_q.put(_TickUpdate(
                    terminal=False, state=FineAlignment.Feedback.PANEL_TOUCHING,
                    message=f"touch detected (Fz={fz:.3f}N) -- starting spiral search",
                ))

        elif self.state == SearchState.PANEL_TOUCHING:
            self.get_logger().info(
                    f"FUCK 1111111111111111111111"
            )
            ...
            self.get_logger().info(
                f"[TOUCH] Surface_height(g)={self.surface_height_g:.5f}m"
            )
            self._feedback_q.put(_TickUpdate(
                terminal=False, state=FineAlignment.Feedback.SEARCHING,
                message=f"Touch the panel at {self.surface_height_g:.5f}m",
            ))

        elif self.state == SearchState.SEARCHING:
            if (self.align_force - fz) > self.hole_force_delta_N or (abs(fx) + abs(fy) > (self.touch_force * 1.5)): #and self._check_steady_state(self._search_z_history, current_z_g):
                self.state = SearchState.HOLE_TESTING
                self._hole_test_z_history.clear()
                self.get_logger().info(
                    f"[CANDIDATE] possible alignment (Fz={fz:.3f}N < "
                    f"align_force={self.align_force:.3f}N) -- testing for a real hole"
                )
                self._feedback_q.put(_TickUpdate(
        ...
        if self.tick % 10 == 0:
            if self.state == SearchState.INSERTING:
                ...
                status = f"PANEL_TOUCHING (surface_g={self.surface_height_g:.5f}m)"

            elif self.state == SearchState.HOLE_TESTING:
                status = f"HOLE_TESTING (current_z_g={current_z_g:.5f}m)"
